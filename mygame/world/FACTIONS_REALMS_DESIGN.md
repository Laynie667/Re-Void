...
4. **Residency + Membership + Ranks** — in progress:
   ...
   - ◑ **4d (started): consent invites + faction pages.** The old ogram "invite" type was pure
     flavour; invites are now **consent-based** — `faction invite`/`faction resident` create a
     *pending* offer (+ an ogram notice), and the recipient runs `faction invites` →
     `faction accept|decline <#>`. Residency invites grant residency **without** faction membership
