...
- `maze gate storm = weather storm` → only during a given global weather state.
- `maze gate deeper = none` → removes the gate.

Condition **types** (current): `conditioning`, `regression`, `devotion`, `standing`, `quota`,
`stat <attr> <min>`, `flag <flag> [value]`, `time <period|season>`, `weather <state>`. Full
catalogue in `gate-conditions.md`. All fail open (§0).
